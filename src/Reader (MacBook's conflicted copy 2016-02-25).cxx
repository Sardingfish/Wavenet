...
Mat<double> Reader::nextGaussian () {
    ...
    if      (shiftIndX < 0) { shiftX.diag( shiftIndX + _size) += 1; }
    else if (shiftIndX > 0) { shiftX.diag( shiftIndX - _size) += 1; }
    if      (shiftIndY < 0) { shiftY.diag( shiftIndY + _size) += 1; }
    else if (shiftIndY > 0) { shiftY.diag( shiftIndY - _size) += 1; }
